...
import { likeMarket, dislikeMarket } from "../../../../apihelpers/helpers";

const DiaryItem = ({location, id, market, likes, dislike}) => {
  ...
  const likeMe = () => {
    ...
    likeMarket(id, userid).then((data) => {
      console.log(data);
      if(data.type === "added"){
        setLike(like+1);
  ...
  const dislikeMe = () => {
    const userid = localStorage.getItem("userId");
    dislikeMarket(id, userid).then((data) => {
      console.log(data);
      if(data.type === "added"){
